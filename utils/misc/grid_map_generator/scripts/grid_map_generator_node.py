...
class GridMapGenerator:
    def __init__(self):
        ...
        if world_name is None or world_name == '':
            rospy.logerr(
                "No world_name argument provided! Please provide the world_name argument.")
            rospy.signal_shutdown("No world_name argument provided.")
            return

        # Create folder inside /arena_simulation_setup/maps
        self.timer0_ = threading.Thread(
            target=self.create_folder, args=(
                world_name,))
        self.timer0_ .start()
        ...
        self.timer1_ = threading.Thread(
            target=self.grid_map_generator, args=())
        self.timer1_ .start()

        # Save the map.pgm and the map.yaml file in maps folder
        self.timer3_ = threading.Thread(
            target=self.save_map, args=(world_name,))
        self.timer3_ .start()

        # Waits for self.timer3_ to finish before proceeding to start
        # self.timer4_
        self.timer3_.join()

        # Write map.pgm to the first line of the map.yaml file
        self.timer4_ = threading.Thread(
            target=self.write_data, args=(world_name,))
        self.timer4_ .start()

        # Waits for self.timer4_ to finish before proceeding to start
        # self.timer5_
        self.timer4_.join()

        # Write map.pgm to the first line of the map.yaml file
        self.timer5_ = threading.Thread(
            target=self.create_file, args=(world_name,))
        self.timer5_ .start()
    ...
    def grid_map_generator(self):
        ...
        try:
            grid_map_generator = rospy.ServiceProxy(
                '/gazebo_2Dmap_plugin/generate_map', Empty)
