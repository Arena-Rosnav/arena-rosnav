...
def count_planner_num(planner_config: dict) -> int:
    return len(
        [
            planner
            for planner in planner_config
            if planner["name"] in MBF_COMPATIBLE_TYPE.LOCAL.__members__
        ]
    )


class SwitchFlatlandEnv(gymnasium.Env):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(SwitchFlatlandEnv, self).__init__()

        self.ns = Namespace(ns)
        self._agent_description = agent_description
        self._planner_configuration = planner_configuration
    ...
    def step(self, action: np.ndarray):
        ...
        reward, reward_info = self.reward_calculator.get_reward(
            action=obs_dict[OBS_DICT_KEYS.CURR_ACTION],
            **obs_dict,
        )
        self._last_action = obs_dict[OBS_DICT_KEYS.CURR_ACTION]
