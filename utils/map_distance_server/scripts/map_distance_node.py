...
class MapDistanceServer:
    ...
    def produce_distance_map(self, new_map: bool = False):
        """Generates and saves or loads the distance map.

        Args:
            new_map (bool, optional): If a new map was recently generated by the MapGenerator.
                Defaults to False.
        """
        if not new_map:
            # only update the map if a new map was not generated
            self.map = self.map_service().map

        if not os.path.exists(self._distance_map_path) or new_map:
    ...
    def _map_callback(self, msg: OccupancyGrid):
        ...
        self.produce_distance_map(new_map=True)
