...
from ArenaScenario import PedsimAgent
...
def read_scenario_file(scenario_file_path):
...
def add_actor_element_person(j, agent=None):
    x = y = 0.0
    z = -10.0
    name = str(j)
    ...
    if agent is not None:
        ...
        y = agent.pos[1]
        z = 0.0
        name = str(j) + "_" + agent.name
    
    actor = Element("actor", name="person_" + name)
    skin = Element("skin")

    # setting start position
    init_pose = etree.fromstring(
        f"<pose> {x} {y} {z} 0.0 0.0 0.0 </pose>"
    )
...
if __name__ == "__main__":
    ...
    if mode in ["scenario", "scenario_staged"]:
        ...
        scenario = read_scenario_file(scenario_path)
        pedsim_agents = [PedsimAgent.fromDict(
                a) for a in scenario["obstacles"]["dynamic"]]

        # looping through the data of every individual dynamic obstacle defined in the scenario file
        for j, ped in enumerate(pedsim_agents):
    ...
    # in the case of for example random-mode the trajectory points of dynamic obstacles are determined randomly in the task_generator node. The position of the actors is determined by pedsim. Therefore we include here the actor model without trajectory points
        ...
        for item in range(num_of_actors):
            actor, _ = add_actor_element_person(item)
