...
class Mod_DynamicMap(TM_Module, Node):
    def __init__(self, **kwargs):
        ...
        Node.__init__(self, 'mod_dynamic_map_node')

        self.callback_group = ReentrantCallbackGroup()

        self.__map_request_pub = self.create_publisher(String, self.TOPIC_REQUEST_MAP, 1)
        self.__task_reset_pub = self.create_publisher(String, self.TOPIC_RESET, 1)

        self.create_subscription(String, self.TOPIC_RESET, self._cb_task_reset, 1, callback_group=self.callback_group)

        self.__get_dist_map_service = self.create_client(GetDistanceMap, self.SERVICE_DISTANCE_MAP, callback_group=self.callback_group)

        self.declare_parameter('num_envs', 1)
        self.declare_parameter(MAP_GENERATOR_NS('episode_per_map'), 1)

        num_envs = self.get_parameter('num_envs').value if "eval_sim" not in self._TASK.robot_managers[0].namespace else 1
        self._target_eps_num = self.get_parameter(MAP_GENERATOR_NS('episode_per_map')).value * num_envs
    ...
    def _set_config(self, config: DynamicMapConfiguration):
        generator = self.get_parameter(self.PARAM_GENERATOR).value
        ...
        for key, value in config_generator.items():
            log += f"\t{key}={value}"
            self.set_parameter(f"{self.PARAM_GENERATOR_CONFIGS}.{generator}.{key}", value)
    ...
    def _update_map(self):
        future = self.__get_dist_map_service.call_async(GetDistanceMap.Request())
    ...
    def request_new_map(self):
        ...
        try:
            ...
            self.get_logger().warn(f"[Map Generator] Timeout while waiting for new map. Continue with current map. {e}")
        else:
