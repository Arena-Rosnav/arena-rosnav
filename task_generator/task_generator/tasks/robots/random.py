...
class TM_Random(TM_Robots):
    def reset(self, **kwargs):
        ...
        ROBOT_POSITIONS: List[Tuple[PositionOrientation, PositionOrientation]] = kwargs.get("ROBOT_POSITIONS", [])
        ...
        if len(ROBOT_POSITIONS) < len(self._PROPS.robot_managers):
            generated_positions = [
                PositionOrientation(position.x, position.y, random.random() * 2*math.pi)
                for position in (
