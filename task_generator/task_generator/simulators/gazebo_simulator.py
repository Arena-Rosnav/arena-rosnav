...
from pedsim_srvs.srv import (
    ...
    SpawnInteractiveObstaclesRequest,
    SpawnObstacle,
    SpawnObstacleRequest,
    SpawnPeds,
    SpawnPed,
)
...
class GazeboSimulator(BaseSimulator):
    def __init__(self, namespace):
        ...
        if rospy.get_param("pedsim"):
            ...
            rospy.Subscriber(
                ...
                Waypoints,
                self.interactive_actor_poses_callback,
            )
            rospy.Subscriber(
                "/pedsim_simulator/simulated_agents",
                AgentStates,
                self.dynamic_actor_poses_callback,
            )
        ...
        if rospy.get_param("respawn_static"):
            for actor in actors.waypoints:
                if "interactive" in actor.name:
                    ...
                    self.spawn_model(
                        actor_name, self.xml_string, "", model_pose, "world"
                    )
    ...
    def create_pedsim_static_obstacle(self, i, map_manager, forbidden_zones):
        ...
        [x, y, theta] = self.map_manager.get_random_pos_on_map(
            safe_distance, forbidden_zones
        )  # check later for the need of free indicies and map papram
        # print(obstacles[i])
    ...
    def create_pedsim_dynamic_obstacle(self, i, map_manager, forbidden_zones):
        ...
        for j in range(10):  # noote was 1000
            dist = 0
            while dist < 8:
                [x2, y2, theta2] = self.map_manager.get_random_pos_on_map(
                    safe_distance, forbidden_zones
                )
    ...
    def spawn_pedsim_interactive_obstacles(self, obstacles):
        ...
        while i < len(obstacles):
            ...
            self.agent_topic_str += (
                f",{self._ns_prefix}pedsim_static_obstacle_{obstacle[0]}/0"
            )
            msg.type = "shelf"
            # msg.name = "test"
        ...
        while i_curr_try < max_num_try:
            # try to call service
            print("209")
            response = self.__respawn_interactive_obstacles_srv.call(
                srv.InteractiveObstacles
            )
    ...
    def spawn_pedsim_dynamic_obstacles(self, peds):
        ...
        for ped in peds:
            ...
            model_pose = Pose(Point(x=x, y=y, z=0), Quaternion())
            new_xml_string = self.xml_string.replace(
                "0 0 0.75", str(x) + " " + str(y) + " 0.75"
            )
    ...
    def spawn_pedsim_map_borders(self):
        ...
        border_vertex = Utils.generate_map_inner_border(
        ...
        for i in range(size):
            lineObstacle = LineObstacle()
            lineObstacle.start.x, lineObstacle.start.y = (
                border_vertex[i, 0],
                border_vertex[i, 1],
            )
            lineObstacle.end.x, lineObstacle.end.y = (
                border_vertex[(i + 1) % size, 0],
                border_vertex[(i + 1) % size, 1],
            )
            add_pedsim_srv.staticObstacles.obstacles.append(lineObstacle)
        self.__add_obstacle_srv.call(add_pedsim_srv)
