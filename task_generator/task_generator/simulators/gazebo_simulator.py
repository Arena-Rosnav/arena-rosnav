...
@SimulatorFactory.register(Simulator.GAZEBO)
class GazeboSimulator(BaseSimulator):
    ...
    def spawn_entity(self, entity):
        ...
        request.initial_pose = geometry_msgs.Pose(
            position=geometry_msgs.Point(x=entity.position.x, y=entity.position.y, z=0),
            orientation=geometry_msgs.Quaternion(
                *quaternion_from_euler(
                    0.0, 0.0, entity.position.orientation, axes="sxyz"
                )
            ),
        )
        ...
        if isinstance(entity, RobotProps):
            rospy.set_param(
                request.robot_namespace("robot_description"), model.description
            )
            rospy.set_param(
                request.robot_namespace("tf_prefix"), str(request.robot_namespace)
            )
