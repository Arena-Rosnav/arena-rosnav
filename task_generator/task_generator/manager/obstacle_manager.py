...
class ObstacleManager:
    ...
    def start_scenario(self, scenario):
        if rospy.get_param("pedsim"):
            ...
            self.simulator.spawn_pedsim_dynamic_scenario_obstacles(
    ...
    def reset_random(
        ...
        for i in range(dynamic_obstacles):
            ...
            if rospy.get_param("pedsim"):
                x = self.simulator.create_pedsim_dynamic_obstacle(
                    i, self.map_manager, forbidden_zones
                )
                dynamic_obstacles_array = np.vstack((dynamic_obstacles_array, x))

            else:
                obstacles.append(
                    self.simulator.create_dynamic_obstacle(position=position)
                )
        ...
        for i in range(interactive_obstacles):
            ...
            if rospy.get_param("pedsim"):
                x = self.simulator.create_pedsim_interactive_obstacle(
                    i, self.map_manager, forbidden_zones
                )
                ...
                obstacles.append(
                    self.simulator.create_interactive_obstacle(position=position)
                )
        ...
        for i in range(static_obstacles):
            ...
            if rospy.get_param("pedsim"):
                x = self.simulator.create_pedsim_static_obstacle(
                    i, self.map_manager, forbidden_zones
                )
                static_obstacles_array = np.vstack((static_obstacles_array, x))
            else:
                obstacles.append(
                    self.simulator.create_static_obstacle(position=position)
                )
