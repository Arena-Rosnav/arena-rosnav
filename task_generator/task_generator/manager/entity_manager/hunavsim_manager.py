...
class HunavsimManager(EntityManager):
    ...
    def test_hunav_services(self):
        ...
        try:
            ...
            self.node.get_logger().info(f"Sending compute_agents request with {
                len(request.current_agents.agents)} agents")
            ...
            if future.result():
                ...
                self.node.get_logger().info(f"Received response with {
                    len(response.updated_agents.agents)} agents")
        ...
        try:
            ...
            self.node.get_logger().info(
                f"Requesting compute_agent for ID: {
                    request.id}")
        ...
        try:
            ...
            self.node.get_logger().info(
                f"Requesting move_agent for ID: {
                    request.agent_id}")
    ...
    def spawn_dynamic_obstacles(self, obstacles: Collection[DynamicObstacle]):
        ...
        self.node.get_logger().error(f"Attempting to spawn {
            len(list(obstacles))} obstacles")
        ...
        for obstacle in obstacles:
            ...
            try:
                ...
                self.node.get_logger().error(
                    f"Behavior Data - Type: {
                        obstacle.behavior.type}, Config: {
            ...
            try:
                ...
                print("\n--- Closest Obstacles Debug ---")
                print(f"Number of closest obstacles: {
                      len(obstacle.closest_obs)}")
                self.node.get_logger().error(f"Closest obstacles count: {
                    len(obstacle.closest_obs)}")
            ...
            try:
                ...
                self.node.get_logger().error(
                    f"Spawn result for {
                        obstacle.name}: {
                        'success' if spawn_success else 'failed'}")
            except Exception as e:
            ...
            if future.result() is not None:
                ...
                for agent in agents:
                    if agent.id in self._pedestrians:
                        ...
                        animation_factor = self.get_animation_factor(
                            agent.behavior)
    ...
    def move_robot(self, name: str, position: PositionOrientation):
        ...
        try:
            ...
            if future.result() is not None:
                self.node.get_logger().info(
                    f"Successfully moved robot {name}")
            else:
