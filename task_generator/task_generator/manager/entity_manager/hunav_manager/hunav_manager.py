...
class HunavManager(EntityManager):
    ...
    def spawn_dynamic_obstacles(self, obstacles: Collection[DynamicObstacle]):
        ...
        for obstacle in obstacles:
            ...
            try:
                ...
                # Spawn in simulator
                self.node.get_logger().warn(f"Attempting to spawn {obstacle.name} in simulator")
    ...
    def spawn_walls(self, walls: WorldWalls, heightmap: WorldMap):
            """Spawn walls with debug logging"""
            self.node.get_logger().warn("=== SPAWN_WALLS START ===")
            self.node.get_logger().warn(f"Converting walls to obstacle")
            
            # Convert walls to obstacle
            wall_obstacle = walls_to_obstacle(heightmap)
            
            # Spawn wall obstacle
            self.node.get_logger().warn("Attempting to spawn wall obstacle")
            success = self._simulator.spawn_entity(wall_obstacle)
            
            if success:
                self.node.get_logger().warn("Successfully spawned walls")
                self._known_obstacles.create_or_get(
                ...
                self.node.get_logger().error("Failed to spawn walls")
                
            self.node.get_logger().warn("=== SPAWN_WALLS COMPLETE ===")

    def remove_obstacles(self, purge=ObstacleLayer.WORLD):
            """Remove obstacles with debug logging"""
            self.node.get_logger().warn("=== REMOVE_OBSTACLES START ===")
            self.node.get_logger().warn(f"Purge level: {purge}")

            if not self._is_paused:
                self.node.get_logger().warn("Pausing and resetting agents")
                self._is_paused = True
                request = ResetAgents.Request()
                self._reset_agents_client.call(request)

            while self._semaphore_reset:
                self.node.get_logger().warn("Waiting for semaphore reset...")
                time.sleep(0.1)
            ...
            try:
                ...
                self.node.get_logger().warn("Processing obstacles for removal...")

                for obstacle_id, obstacle in self._known_obstacles.items():
                    if purge >= obstacle.layer:
                        self.node.get_logger().warn(f"Marking {obstacle_id} for removal")
                ...
                for obstacle_id in to_forget:
                    self.node.get_logger().warn(f"Forgetting obstacle {obstacle_id}")
            ...
            self.node.get_logger().warn("Executing removal actions...")
            for action in actions:
                action()

            self.node.get_logger().warn("=== REMOVE_OBSTACLES COMPLETE ===")
    ...
    def unuse_obstacles(self):
        """Remove/unuse all obstacles with debug logging"""
        self.node.get_logger().warn("=== UNUSE_OBSTACLES START ===")
        try:
            self.node.get_logger().warn("Removing obstacles...")
            result = self.remove_obstacles(ObstacleLayer.WORLD)
            self.node.get_logger().warn(f"Removal result: {result}")
            return True
        except Exception as e:
            self.node.get_logger().error(f"Error in unuse_obstacles: {str(e)}")
            return False
